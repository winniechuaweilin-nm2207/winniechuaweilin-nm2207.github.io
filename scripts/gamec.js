...
function plantFlag(select) {
    if (select.innerHTML === "" && countflag < 50){
        ...
        if(flagCorrect === 50) {
            showCells();
            setTimeout(function(){
                window.alert("Congrats " + localStorage.getItem("saveName") + ", you saved the Universe in " + (360 - time) + "s! Proud of you buddy!");
            },10);
            musicPlaying = true;
            musicControl();
...
function startTimer() {
    if(time > 0) {
        ...
        //showCells();
        setTimeout(function (){
             window.alert("CHIKABOOM! Times up! Sorry " + localStorage.getItem("saveName") + ", you lost. Try harder next time!");
        },10);
        stopTimer();
        musicPlaying = true;
        musicControl();
